...
namespace ntu.xzmcwjzs.Tests
{
    public abstract class xzmcwjzsTestBase : AbpIntegratedTestBase<xzmcwjzsTestModule>
    {
        ...
        protected void UsingDbContext(Action<xzmcwjzsDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<xzmcwjzsDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<xzmcwjzsDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<xzmcwjzsDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<xzmcwjzsDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<xzmcwjzsDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<xzmcwjzsDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<xzmcwjzsDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<xzmcwjzsDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<xzmcwjzsDbContext, Task<T>> func)
        {
            T result;
